...
parser = argparse.ArgumentParser(description='Decide if an image is a picture of a sign')
...
model.load("classifier.tfl.ckpt")

# Load the image file
img = imageio.imread(args.image)

# Scale it to 32x32
img = imageio.imresize(img, (32, 32)).astype(np.float32)

# Predict
prediction = model.predict([img])

# Check the result.
is_sign = np.argmax(prediction[0]) == 1
