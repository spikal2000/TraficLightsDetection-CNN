...
model.load_state_dict(torch.load('trafic_signs_classification.pth'))  # Load the trained model state dict
model.eval()
...
#image, image1, proposals = selective_search('3.jpg')
#image, image1, proposals = selective_search('STOPsign.jpg')
print(image1)
print(proposals)
...
# Iterate over the proposal regions and perform traffic sign detection and classification
for idx, proposal_region in enumerate(proposals_regions):
    # Preprocess the proposal region
    transform = transforms.Compose([
        transforms.Resize((64, 64)),
        transforms.ToTensor(),
    ])
    proposal_image = Image.fromarray((proposal_region * 255).astype(np.uint8))
    proposal_image = transform(proposal_image)
    proposal_image = proposal_image.unsqueeze(0)

    # Make predictions on the proposal region
    with torch.no_grad():
    ...
    # Check if a traffic sign is detected above the confidence threshold
    if confidence.item() > confidence_threshold:
        predicted_label = predicted.item()
